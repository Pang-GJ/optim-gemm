...
void InnerKernel(int m, int n, int k, float* a, int lda, float* b, int ldb,
  ...
  for (j = 0; j < n; j += 12) {
    ...
    for (i = 0; i < m; i += 8) {
      ...
      Kern_8x12(packedA + i * k, packedB + j * k, k, &C(i, j), ldc, is_first_k);
    }
    if (i != m) {
      if (0 == j) PackMatrixA_4(k, &A(i, 0), lda, packedA + i * k);
      Kern_4x12(packedA + i * k, packedB + j * k, k, &C(i, j), ldc, is_first_k,
                4);
    }
  }
  if (j != n) {
    for (; j < n; j += 4) {
      ...
      for (i = 0; i < m; i += 8) {
        if (i + 8 > m) break;
        Kern_8x4(packedA + i * k, packedB + j * k, k, &C(i, j), ldc, is_first_k,
                 n - j);
      }
      if (i != m) {
        Kern_4x4(packedA + i * k, packedB + j * k, k, &C(i, j), ldc, is_first_k,
                 m - i, n - j);
      }
    }
  }
...
#define LOAD_C                        \
  ...
  asm volatile(
      ...
      : [a_ptr] "+r"(a_ptr), [b_ptr] "+r"(b_ptr), [K] "+r"(K), [LDC] "+r"(LDC),
        [is_first_k] "+r"(is_first_k), [oddk] "+r"(oddk), [outptr] "+r"(outptr)
      :
...
#define STORE_C                  \
  ...
  asm volatile(
      ...
      : [a_ptr] "+r"(a_ptr), [b_ptr] "+r"(b_ptr), [K] "+r"(K), [LDC] "+r"(LDC),
        ...
        [n_remain] "+r"(n_remain)
      :
      : "v0", "v1", "v2", "v5", "v8", "v11", "v14", "v17", "v20", "v23", "v26",
        "v29", "x1", "x2", "x3", "x4", "x5", "x6", "x7", "cc", "memory");
